...
echo "$current_costs" | jq -c '.ResultsByTime[].Groups[]' | while read -r line; do
    service=$(echo $line | jq -r '.Keys[0]')
    current_cost=$(echo $line | jq -r '.Metrics.AmortizedCost.Amount | tonumber')

    forecast=$(aws ce get-cost-forecast --time-period Start=$current_day,End=$end_of_month --granularity MONTHLY --metric "AMORTIZED_COST" --filter '{"Dimensions": {"Key": "SERVICE", "Values": ["'$service'"]}}')
